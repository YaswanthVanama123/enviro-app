...
export function getPricingTierColor(tier: string): string {
  ...
  if (tier.includes('110%')) return '#0d9488';
  if (tier.includes('Redline')) return '#6b7280';
  if (tier.includes('Below')) return '#dc2626';
  return '#6b7280';
}
...
export const ACCOUNT_TYPE_OPTIONS: {value: AccountType; label: string; description: string}[] = [
  ...
  {value: 'Bread15', label: 'Bread15', description: 'Within 15 min of Anchor (-$75 deduction)'},
  {value: 'Pit', label: 'Pit', description: 'Far from Anchor (-$100 deduction)'},
];
...
export const PRICING_LINE_OPTIONS: {value: PricingLine; label: string; description: string}[] = [
  {value: 'Redline', label: 'Redline', description: 'Standard pricing (1x quota)'},
  {value: 'Greenline', label: 'Greenline', description: '130%+ premium (2x quota)'},
];
