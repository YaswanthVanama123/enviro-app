...
export const carpetPricingConfig = {
  ...
  perVisitMinimum: 250,
  installMultipliers: {dirty: 3, clean: 1},
  billingConversions: {
  ...
  } as Record<CarpetFrequency, {annualMultiplier: number; monthlyMultiplier: number}>,
  minContractMonths: 2,
  ...
  } as Record<CarpetFrequency, {visitsPerYear: number; monthlyMultiplier: number}>,
};
...
export interface CarpetFormState {
...
function transformBackendFrequencyMeta(
  ...
  if (!backendMeta) {
    return carpetPricingConfig.frequencyMeta;
  }
...
export function computeCarpetCalc(
  ...
  const isVisitBasedFrequency = infrequentFrequencies.includes(freq);
  ...
  if (serviceActive) {
    if (freq === 'oneTime') {
      calculatedFirstMonthTotal =
        form.includeInstall && installOneTime > 0 ? installOneTime : perVisitCharge;
    } else if (freq === 'weekly') {
  ...
  if (contractMonths > 0 && serviceActive) {
    if (freq === 'oneTime') {
      ...
      const effectiveMonthlyVisits =
        backendConfig?.frequencyMetadata?.weekly?.monthlyRecurringMultiplier ?? monthlyVisits;
      ...
      const effectiveMonthlyVisits =
        backendConfig?.frequencyMetadata?.biweekly?.monthlyRecurringMultiplier ?? monthlyVisits;
      totalVisitsForContract = Math.round(contractMonths * effectiveMonthlyVisits);
      if (form.includeInstall && installOneTime > 0) {
        const remainingMonths = Math.max(contractMonths - 1, 0);
        calculatedContractTotal = firstMonthTotal + remainingMonths * monthlyRecurring;
      ...
      const cycleMonths = backendConfig?.frequencyMetadata?.monthly?.cycleMonths ?? 1;
  ...
  if (serviceActive && contractMonths > 0) {
    ...
    if (freq === 'oneTime') {
      ...
      if (form.includeInstall && baselineInstallOneTime > 0) {
        const backendMeta = backendConfig?.frequencyMetadata?.[
          freq as keyof NonNullable<BackendCarpetConfig['frequencyMetadata']>
        ];
        const extraVisits =
