// Foaming Drain calculation — ported 1:1 from the web app
// (enviromaster-webapp/src/features/services/kinds/foamingDrain/compute.ts
//  + components/services/foamingDrain/foamingDrainConfig.ts)
...
export const FOAMING_DRAIN_CONFIG = {
  ...
  volumePricing: {
    minimumDrains: 10,
    weekly: {ratePerDrain: 20},
    bimonthly: {ratePerDrain: 10},
  },
  grease: {weeklyRatePerTrap: 125, installPerTrap: 300},
  green: {weeklyRatePerDrain: 5, installPerDrain: 100},
  plumbing: {weeklyAddonPerDrain: 10},
  installationRules: {filthyMultiplier: 3},
  tripCharges: {standard: 0, beltway: 0},
  billingConversions: {
    oneTime: {monthlyMultiplier: 0},
    weekly: {monthlyVisits: 4.33, monthlyMultiplier: 4.33},
    biweekly: {monthlyMultiplier: 2.165},
    twicePerMonth: {monthlyMultiplier: 2.0},
    monthly: {monthlyMultiplier: 1.0},
    everyFourWeeks: {monthlyMultiplier: 1.0833},
    bimonthly: {monthlyMultiplier: 0.5},
    quarterly: {monthlyMultiplier: 0.333},
    biannual: {monthlyMultiplier: 0.167},
    annual: {monthlyMultiplier: 0.083},
    actualWeeksPerYear: 52,
    actualWeeksPerMonth: 4.33,
  } as any,
  contract: {minMonths: 2, maxMonths: 36, defaultMonths: 12},
  defaultFrequency: 'weekly' as FoamingDrainFrequency,
  allowedFrequencies: [
    'oneTime',
    'weekly',
    'biweekly',
...
export interface FoamingDrainFormState {
  ...
  facilityCondition: 'normal' | 'filthy';
  location: string;
...
export interface FoamingDrainQuoteResult {
  frequency: FoamingDrainFrequency;
...
function transformBackendFrequencyMeta(backendMeta: BackendFoamingDrainConfig['frequencyMetadata'] | undefined) {
  ...
  for (const freq of cycleBased) {
    ...
    if (data?.cycleMonths) {
      transformedBilling[freq] = {cycleMonths: data.cycleMonths, monthlyMultiplier: 1 / data.cycleMonths};
    }
  }
...
export function buildFoamingDrainActiveConfig(
  ...
  return {
    ...
    volumePricing: {
      ...
      bimonthlyRatePerDrain:
        backendConfig?.volumePricing?.bimonthlyRatePerDrain ?? cfg.volumePricing.bimonthly.ratePerDrain,
    },
    grease: {
...
export function computeFoamingDrainQuote(
  ...
  const location = state.location || 'standard';
  const condition = state.facilityCondition || 'normal';

  const isVolume = standardDrains >= activeConfig.volumePricing.minimumDrains;
  const canUseInstallProgram = isVolume && !state.useBigAccountTenWeekly && !state.isAllInclusive;

  const installDrains = canUseInstallProgram ? Math.min(installRequested, standardDrains) : 0;
  ...
  const altTotal =
    standardDrainsActive > 0 ? effectiveAltBaseCharge + effectiveAltExtraPerDrain * standardDrainsActive : 0;
  ...
  if (standardDrainsActive > 0 && !state.isAllInclusive) {
    ...
    } else if (altTotal > 0 && altTotal < tenTotal) {
      useAltPricing = true;
      usedSmallAlt = true;
    } else {
      useAltPricing = false;
  ...
  const weeklyStandardDrains = state.isAllInclusive ? 0 : useAltPricing ? altTotal : tenTotal;
  ...
  if (installDrains > 0 && canUseInstallProgram) {
    ...
    const perDrainRate =
      state.installFrequency === 'bimonthly' ? effectiveVolumeBimonthlyRate : effectiveVolumeWeeklyRate;
    weeklyInstallDrains = perDrainRate * installDrains;
  }

  const weeklyPlumbing =
    state.needsPlumbing && plumbingDrains > 0 ? plumbingDrains * effectivePlumbingAddonRate : 0;
  ...
  const weeklyServiceRaw =
    weeklyStandardDrains + weeklyInstallDrains + weeklyPlumbing + weeklyGreaseTraps + weeklyGreenDrains;
  ...
  if (condition === 'filthy' && standardDrainsActive > 0 && !state.useBigAccountTenWeekly) {
    const filthyDrainCount =
      filthyDrains > 0 && filthyDrains <= standardDrainsActive ? filthyDrains : standardDrainsActive;
    let weeklyFilthyCost = 0;
    if (useAltPricing) {
      weeklyFilthyCost = effectiveAltBaseCharge + effectiveAltExtraPerDrain * filthyDrainCount;
    } else {
  ...
  const greaseInstallOneTime =
    state.chargeGreaseTrapInstall && greaseTraps > 0 ? effectiveGreaseInstallRate * greaseTraps : 0;
  ...
  const originalContractTotal = (() => {
    ...
    const baselineStandardDrains = state.isAllInclusive ? 0 : baselineUseAlt ? bAltTotal : bTenTotal;
    ...
    const baselinePlumbing =
      state.needsPlumbing && plumbingDrains > 0 ? plumbingDrains * activeConfig.plumbing.weeklyAddonPerDrain : 0;
    ...
    if (baselineInstallation > 0) {
      ...
      if (condition === 'normal') {
        baselineFirstVisitService += baselineStandardDrains;
      }
      if (!state.chargeGreaseTrapInstall) {
        baselineFirstVisitService += baselineGrease;
      }
  ...
  return {
    ...
    annualRecurring: state.customContractTotal ?? contractTotalWithCustomFields,
